...
def load_dataset(dataset_name, root_dir="data", embedding_type='llm'):
    ...
    Wrr = [drug_ChemS, drug_AtcS, drug_SideS, drug_DDIS, drug_TargetS, drug_GepS, drug_KgS, drug_LlmS]  # Six drug similarity matrices including gene expression similarity
    Wrr_six_geps = [drug_ChemS, drug_AtcS, drug_SideS, drug_DDIS, drug_TargetS, drug_GepS]
    Wrr_six_kgs = [drug_ChemS, drug_AtcS, drug_SideS, drug_DDIS, drug_TargetS, drug_KgS]
    Wrr_six_llms = [drug_ChemS, drug_AtcS, drug_SideS, drug_DDIS, drug_TargetS, drug_LlmS]
    Wrr_five = [drug_ChemS, drug_AtcS, drug_SideS, drug_DDIS, drug_TargetS]  # Five drug similarity matrices used in previous studies
    Wdd = [disease_PhS, disease_DoS, disease_LlmS]  # Disease similarity matrices
    Wdd_two = [disease_PhS, disease_DoS]

    # Combine drug and disease similarity matrices into 3D arrays (dimensions: n x n x 6)
    Trr = np.stack((drug_ChemS, drug_AtcS, drug_SideS, drug_DDIS, drug_TargetS, drug_GepS, drug_KgS, drug_LlmS), axis=2)  # Six drug similarity matrices (3D array)
    ...
    Tdd = np.stack((disease_PhS, disease_DoS, disease_LlmS), axis=2)  # Two disease similarity matrices (3D array)
    ...
    return (Drug_name, Disease_name, Wrd, 
            Wrr, Wrr_six_geps, Wrr_six_kgs, Wrr_six_llms, Wrr_five, 
            Wdd, Wdd_two, 
            Trr, Trr_six_geps, Trr_six_kgs, Trr_six_llms, Trr_five,
            Tdd, Tdd_two, 
...
def extract_embeddings(dataset_name, merged_data, drug_embeddings, disease_embeddings):
    """
    ...
    drug_file_path = f'data/{dataset_name}/{dataset_name}_drug_embedding.csv'
    pd.DataFrame(small_drug_embeddings).to_csv(drug_file_path, index=False)
    
    # Save the extracted disease embeddings to a CSV file
    disease_file_path = f'data/{dataset_name}/{dataset_name}_disease_embedding.csv'
